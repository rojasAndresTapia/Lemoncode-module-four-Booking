...
const handleNightsInput = () => {
  ...
  if (nights == 'Undefined' || nights == 'NaN') {
    ...
    nights = 70 * parseInt(document.getElementById('nights').value);
  }
...
const handleParkingInput = () => {
  ...
  if (parkingNights == 'Undefined' || parkingNights == 'NaN') {
    ...
    parkingNights = 10 * parseInt(document.getElementById('parking').value);
  }
...
const handleButton = () => {
  resultValue =
    parseInt(handleRoomInput()) +
    parseInt(handleDoubleOrSingleInput()) +
    parseInt(handleParkingInput()) +
    parseInt(handleNightsInput()) +
    parseInt(handleSpaCheckbox());
  resultInputElement.value = resultValue;
  resultInputElement.innerHTML = 'Total: ' + resultValue + '€';
};
...
const calculate = (response) =>
  alert('El resultado de la operación es: ' + resultValue);
